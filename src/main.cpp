...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SamCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SamCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    // Subsidy is cut in half every 120960 blocks, which will occur approximately every 6 months
    nSubsidy >>= (nHeight / 120960); // Samcoin: 120960k blocks in ~6 months

    if(nHeight == 1337) // no block reward within the first 3 days
        nSubsidy = 5 * 1337;
...
static const int64 nTargetTimespan = 6 * 60 * 60; // SamCoin: Diff retarget - 6 hours
static const int64 nTargetSpacing = 120; // SamCoin: 2minutes block time
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // SamCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal samcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SamCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5d3cc41402e1e2ac1e9cbe20d6233aa698eeaa3dacd19f7c8c8db76710430e1f");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "if you immediately know the candlelight is fire";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388889827;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2237314;
        ...
        assert(block.hashMerkleRoot == uint256("0x35b74eae0b634f712e9ee60655f7f13b82d5affd1f3e6715ed546910619c2589"));
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // SamCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
